...
VERSION_NOTES = """
v2.0.0 (2026-01-25)
- ★ AI 사전 기능: 자연어로 용어 추가 ("겉감을 영어로 shell fabric으로 추가해줘")
- ★ 번역 프롬프트 강화: 봉제/의류 전문 컨텍스트 추가
- ★ 안정화: 번역 정상작동, AI사전 정상작동
- ⏳ 번역 후 글자겹침 문제 - 미해결
...
def draw_arrow_on_image(draw, start_x, start_y, end_x, end_y, color=(255, 107, 107), width=2):
    ...
    # 선 그리기
    draw.line([(start_x, start_y), (end_x, end_y)], fill=color + (255,), width=width)
    ...
    # 화살촉 좌우 점 계산
    left_x = end_x - arrow_length * (ux * math.cos(arrow_angle) - uy * math.sin(arrow_angle))
    left_y = end_y - arrow_length * (uy * math.cos(arrow_angle) + ux * math.sin(arrow_angle))
    right_x = end_x - arrow_length * (ux * math.cos(arrow_angle) + uy * math.sin(arrow_angle))
    right_y = end_y - arrow_length * (uy * math.cos(arrow_angle) - ux * math.sin(arrow_angle))
    
    # 화살촉 삼각형 그리기
    draw.polygon([(end_x, end_y), (left_x, left_y), (right_x, right_y)], fill=color + (255,))
...
def render_memos_on_image(img, memos):
    ...
    # ★ 1단계: 화살표 먼저 그리기 (메모 아래에 위치)
    for memo in memos:
        if not isinstance(memo, dict):
            continue
        arrow = memo.get("arrow")
        if not arrow:
            continue
        
        # 메모 시작 위치 (비율 → 픽셀)
        memo_x = clamp(safe_float(memo.get("x"), 0.0), 0.0, 1.0) * base.size[0]
        memo_y = clamp(safe_float(memo.get("y"), 0.0), 0.0, 1.0) * base.size[1]
...
def generate_preview_image(image_base64, translations, target_lang='english', memos=None):
    """미리보기 이미지 생성 (메모리에서 처리) - v2.0.0 (겹침 감지 + 약어 + 메모/화살표)"""
    ...
    # ★ 메모 및 화살표 렌더링
    if memos:
        img_result = render_memos_on_image(img_result, memos)
...
<body>
    ...
    <script>
        ...
        // 줌 적용 함수 - previewStage 전체에 적용 (이미지+화살표+메모 동시 확대)
        function applyZoom(zoom) {
            currentZoom = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, zoom));
            previewStage.style.transform = `scale(${currentZoom / 100})`;
        ...
        async function showPreviewImage(pageIdx, forceRefresh = false) {
            ...
            try {
                ...
                console.log('[Preview Debug] memos count:', page.memos ? page.memos.length : 0);
                ...
                const response = await fetch('/generate_preview', {
                    ...
                    body: JSON.stringify({
                        ...
                        memos: page.memos || []  // ★ 메모/화살표 데이터 전송
                    })
                });
        ...
        // ★ 미리보기 이미지 새로고침 (메모/화살표 저장 후 호출)
        async function refreshPreviewImage() {
            const page = pagesData[currentPage];
            if (!page || !page.image || !page.translations) return;
            ...
            try {
                ...
                const data = await response.json();
                if (data.success) {
                    previewCache[currentPage] = data.preview;
                    previewImg.src = 'data:image/png;base64,' + data.preview;
                }
            } catch (error) {
...
def generate_preview():
    ...
    try:
        ...
        target_lang = data.get('target_lang', 'english')
        memos = data.get('memos', [])  # ★ 메모/화살표 데이터

        print(f"[generate_preview] Received {len(translations)} translations, {len(memos)} memos, target_lang={target_lang}")
        ...
        # 미리보기 이미지 생성 (메모/화살표 포함)
        print("[generate_preview] Calling generate_preview_image...")
        preview_base64 = generate_preview_image(image_base64, translations, target_lang, memos)
