# -*- coding: utf-8 -*-
"""
Comet + ERP Table Extractor v3.0
================================
...
VISION_MODEL = "qwen2.5vl:latest"  # 7B 모델 (6GB) - moondream보다 강력, 32b보다 빠름
...
def apply_corrections_to_table(table_2d: list, spelling_corrections: list, missing_cells: list) -> tuple:
    ...
    for missing in missing_cells:
        row_idx = missing.get("row", 0) - 1  # 1-based → 0-based
        col_idx = missing.get("col", 0) - 1
...
def generate_result_html(tables: list, validation: dict = None, ai_result: dict = None, differences: list = None) -> str:
    """
    추출된 테이블을 HTML로 렌더링 (3단 수직 배치)
    ...
    Args:
        ...
        validation: AI 검증 결과
        ai_result: AI 재추출 결과 (Progressive Rendering용)
        differences: OCR vs AI 차이점 목록
    """
    if not tables:
    ...
    for table in tables:
        ...
        <div class="table-block section-original">
            <div class="table-header">
                <span class="table-title">📋 1. Original OCR Table</span>
        ...
        # ============= 섹션 2: AI 보정 과정 (Round 25 - 타겟팅된 검사) =============
        # differences가 비어있어도 AI 검사 결과는 표시 (검사 완료 메시지)
        if ai_result and ai_result.get('success'):
            ...
            <div class="table-block section-diff">
                <div class="table-header">
                    <span class="table-title">🔍 2. AI Targeted Check (Round 25)</span>
            ...
            if diff_count > 0:
                ...
                <div class="table-block section-final">
                    <div class="table-header">
                        <span class="table-title">✅ 3. Final Corrected Table</span>
            ...
            <div class="table-block section-diff">
                <div class="table-header">
                    <span class="table-title">⚠️ 2. AI Correction Failed</span>
                </div>
    ...
    if validation:
        if validation.get('errors'):
            ...
            <div class="validation-box error">
                ...
                <ul>{''.join(f'<li>{e}</li>' for e in validation['errors'])}</ul>
            </div>
            """)
        if validation.get('warnings'):
            ...
            <div class="validation-box warning">
                ...
                <ul>{''.join(f'<li>{w}</li>' for w in validation['warnings'])}</ul>
            </div>
            """)
...
def process_image(img: Image.Image, img_base64: str, use_ai_correction: bool = True) -> dict:
    """
    이미지 처리 메인 함수 (Round 24 합의 - AI 보정 포함)

    Args:
        img: PIL Image
        img_base64: Base64 인코딩된 이미지
        use_ai_correction: AI 보정 사용 여부 (기본: True)
    """
    result = {
        'success': False,
        'tables': [],
    ...
    try:
        ...
        # 2단계: AI 보정 (use_ai_correction이 True인 경우)
        # Round 25: 타겟팅된 AI 검사 (한글 철자 + 누락 셀만)
        ai_result = None
        differences = None

        if use_ai_correction and tables:
            # OCR HTML을 2D 배열로 변환
            ocr_2d = html_to_2d_array(tables[0]['html'])

            # AI 타겟팅 검사 (한글 철자 오류 + 누락 셀)
            ai_result = ai_targeted_check(img, ocr_2d)
            result['ai_result'] = ai_result

            if ai_result.get('success'):
                # 수정사항 적용 및 차이점 생성
                spelling_corrections = ai_result.get('spelling_corrections', [])
                missing_cells = ai_result.get('missing_cells', [])

                corrected_table, differences = apply_corrections_to_table(
                    ocr_2d, spelling_corrections, missing_cells
                )
                result['differences'] = differences
                result['corrected_table'] = corrected_table

        # 3단계: HTML 생성 (차이점 하이라이트 포함)
        result['html'] = generate_result_html(
            tables,
...
def process_image_ocr_only(img: Image.Image) -> dict:
    """
    OCR만 수행 (Progressive Rendering 1단계용)
    AI 보정 없이 빠르게 결과 반환
    """
    result = {
        'success': False,
    ...
    try:
        ...
        result['tables'] = tables
        result['success'] = True
        result['html'] = generate_result_html(tables)
...
# 새로운 UI 레이아웃 - 상단 바에 업로드, 수직 스크롤 레이아웃
HTML_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Comet Table Extractor v3.0</title>
    <style>
        * {
...
<body>
    ...
    <div class="main-container">
        ...
        <div id="result-area">
            ...
            <div id="result-html" style="display: none;"></div>
        </div>
    </div>
...
def api_extract_ocr():
    """
    OCR만 수행 (Progressive Rendering 1단계)
    AI 보정 없이 빠르게 결과 반환
    """
    if 'image' not in request.files:
        return jsonify({'success': False, 'error': 'No image provided'})
...
def api_extract_ai():
    ...
    try:
        ...
        ocr_2d = html_to_2d_array(ocr_html)

        # 타겟팅된 AI 검사 (한글 철자 + 누락 셀만)
        ai_check_result = ai_targeted_check(img, ocr_2d)

        if not ai_check_result.get('success'):
            return jsonify({
                'success': False,
                'error': ai_check_result.get('error', 'AI check failed')
            })

        # 수정사항 적용
        spelling_corrections = ai_check_result.get('spelling_corrections', [])
        missing_cells = ai_check_result.get('missing_cells', [])
        ...
        return jsonify({
            ...
            'ai_result': {
                ...
                'spelling_corrections': spelling_corrections,
                'missing_cells': missing_cells,
                'korean_texts_checked': ai_check_result.get('korean_texts_checked', 0),
                'empty_cells_checked': ai_check_result.get('empty_cells_checked', 0)
            },
            'differences': differences,
            'diff_count': len(differences),
            'correction_type': 'targeted'  # Round 25 방식 표시
        })
...
def health():
    return jsonify({
        'status': 'ok',
        'version': '3.0',
        'engine': 'img2table + PaddleOCR + Qwen2.5-VL',
        'features': ['OCR', 'AI Correction', 'Progressive Rendering', 'Unified Diff']
    })


if __name__ == '__main__':
    print("=" * 60)
    print("Comet Table Extractor v3.0")
    print("=" * 60)
    print("Engine: img2table + PaddleOCR + Qwen2.5-VL")
    print("Port: 6004")
    print("UI: Full-screen table view")
    print("=" * 60)
